...
async function chatViaProvider({
  ...
  if (provider === 'modelscope') {
    return modelscopeChat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'ollama') {
    return ollamaChat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'kilo') {
    return kiloChat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'llm7') {
    return llm7Chat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'ovhcloud') {
    return ovhcloudChat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'huggingface') {
    return huggingfaceChat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'dashscope') {
    return dashscopeChat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'cloudflare') {
    return cloudflareChat({ apiKey, modelId, messages, maxTokens, tools });
  }
  if (provider === 'mistral') {
  ...
  if (provider === 'cursor') {
    ...
    return cursorChat({ apiKey, modelId, messages });
  }
