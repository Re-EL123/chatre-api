...
const { openrouterChat, openaiChat } = require('./providers/openai-compat');
...
async function chatViaProvider({
  ...
  if (provider === 'openrouter') {
    return openrouterChat({ apiKey, modelId, messages, maxTokens });
  }
  if (provider === 'openai') {
    return openaiChat({ apiKey, modelId, messages, maxTokens });
  }
...
async function chatWorkerWithTools({
  ...
  if (provider === 'chatre') {
    ...
    // BYOK providers: text-only; agent falls back to text tool protocol
