...
/**
 * Workers AI chat models only accept {role, content:string} — no tool role,
 * tool_calls, or content arrays. Convert OpenAI-style history into text.
 */
function toWorkersAiMessages(messages) {
  const out = [];
      ...
      const blocks = m.tool_calls
        .map((tc) => {
          ...
          return (
            '```tool\n' +
            JSON.stringify({
              tool: fn.name || (tc && tc.name) || 'unknown',
...
async function chatWorker({
  ...
  // Workers AI does not reliably support OpenAI tool schemas / tool roles.
  // Always send sanitized string messages; never attach native tools.
  const body = {
  ...
  // Intentionally ignore `tools` — Chatre agent uses ```tool text protocol on Workers.
  ...
  if (stream && resolvedMode !== 'analyst' && resolvedMode !== 'critic') return res;
...
async function chatWorkerWithTools({
  ...
  if (provider === 'chatre') {
    try {
      // Workers AI: ```tool text protocol only (no native tools / tool roles).
    ...
    const passTools =
      ...
      provider === 'anthropic' ||
      provider === 'google' ||
      // Free gateways: broken/partial tools schema (array content + oneOf prompt errors).
      // Use the ```tool text protocol instead of native function calling.
      provider === 'llm7' ||
      provider === 'kilo' ||
      provider === 'ovhcloud'
        ? undefined
