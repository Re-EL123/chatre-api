...
function finishGate({ state, todos, step, forcePlan, taskType, deliverySuccess }) {
  ...
  if (kind === 'build' || kind === 'debug' || kind === 'git') {
    ...
    if (fileOk && !state.verified && (kind === 'build' || kind === 'debug') && step < 8) {
      return (
        '[internal] Run preview_project on /home/user/projects/<slug> now (live localhost preview + debug). ' +
        'If it returns errors, patch_file / write_file to fix them and re-run preview_project. ' +
        'Do not claim done until preview_project returns ok. Do not narrate this message.'
      );
    }
    if (fileOk && state.previewFailed) {
      return (
        '[internal] Live preview/debug still has errors. Fix them with patch_file or write_file, then call preview_project again. Do not narrate this message.'
      );
    }
    return null;
  }
