...
function finishGate({ state, todos, step, forcePlan }) {
  ...
  if (!state.explored && step < 3) {
    return 'Continue: inspect first (view_tree, read_file, navigate, read_page, search_web, or http_request).';
  }
  if (!state.planned && step < 4) {
    return 'Continue: plan + todo_write / todo before finishing.';
  }
  if (state.implemented && !state.verified) {
    return 'Continue: verify (verify_project, execute_command, read_page, or screenshot), then finish.';
  }
