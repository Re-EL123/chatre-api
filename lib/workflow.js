...
/**
 * OpenCode-inspired Build agent workflow for Chatre.
 * Source of truth: OpenCode "beast" / Build agent loop —
 * investigate → plan with todos → implement → verify → keep going until done.
 */

const OPENCODE_WORKFLOW_PROMPT = `You are Chatre, an OpenCode-style BUILD agent. Keep working until the user's request is fully solved. Do not hand control back early.
...
5. Before finishing, you MUST verify (verify_project and/or execute_command). If verification fails, fix and retry.
...
8. Casual chat may answer without tools. Any build/code/docs/git task MUST use the workflow below.
...
4. **Implement** — one task at a time: write_file / append_file / create_directory / execute_command as needed. Mark each todo done when finished.
5. **Verify** — run verify_project and relevant execute_command / run_javascript / run_python checks.
...
8. **Finish** — only after all todos are done AND verification passed. Final summary with paths and how to run. No more tool calls in the final summary turn.
...
Keep todos short, atomic, and verifiable.
...
function recordToolPhase(state, tool) {
  ...
  if (
    ...
    // execute_command counts as verify only after implement started
...
function finishGate({ state, todos, step, forcePlan }) {
  ...
  if (!state.explored && step < 3) {
    return (
      'OpenCode workflow gate: you have not explored the workspace yet. ' +
      'Call view_tree or list_directory / read_file first, then continue. Do not finish yet.'
    );
  }
  ...
  if (state.implemented && !state.verified) {
    return (
      ...
      'Run verify_project and/or execute_command to prove it works, then finish.'
    );
  }
  if (!state.implemented && step < 5) {
    return (
      'OpenCode workflow gate: nothing was implemented yet. Use write_file / execute_command to deliver the work.'
    );
  }
  return null;
}
