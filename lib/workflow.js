...
function recordToolPhase(state, tool) {
  ...
  if (
    ...
    if (tool === 'verify_project' || tool === 'list_directory' || tool === 'view_tree') {
      s.verified = true;
 ...
 */
function finishGate({ state, todos, step, forcePlan, taskType, deliverySuccess }) {
  ...
  const delivered = !!deliverySuccess || !!state.implemented;
  ...
  // Build/debug: NEVER finish without a real write_file (create_directory alone is not enough).
  if (kind === 'build' || kind === 'debug' || kind === 'run' || kind === 'git') {
    if (!delivered) {
      return (
        ...
        'Chat dumps and Python open() do NOT create files. Do not narrate this message.'
      );
    }
    if (delivered && !state.verified && (kind === 'build' || kind === 'debug') && step < 6) {
      return (
        '[internal] Confirm with list_directory on the project folder (or verify_project), then finish with <answer> listing real paths. Do not narrate this message.'
      );
    }
    return null;
  }
