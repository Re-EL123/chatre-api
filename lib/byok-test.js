...
const { cursorMe } = require('./providers/cursor');
...
function validateProviderKey(provider, apiKey) {
  ...
  if (p === 'cursor') {
    if (!/^(key_|crsr_)/i.test(key) && key.length < 20) {
      return {
        ok: false,
        error:
          'Cursor API keys come from https://cursor.com/dashboard/api (often key_… / crsr_…). Paste the full key.',
      };
    }
    if (key.length < 20) {
      return {
        ok: false,
        error: 'Cursor API key looks truncated. Paste the full key from the Cursor dashboard.',
      };
    }
  }
  return { ok: true, key };
...
async function testByokProvider(uid, provider, overrideKey) {
  ...
  try {
    ...
    if (p === 'openrouter') {
      ...
      const me = await cursorMe(apiKey);
      result = {
        response:
          'ok' +
          (me && me.userEmail ? ' (' + me.userEmail + ')' : me && me.apiKeyName ? ' (' + me.apiKeyName + ')' : ''),
      };
    } else {
