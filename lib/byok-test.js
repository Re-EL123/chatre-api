...
function validateProviderKey(provider, apiKey) {
  ...
  if (p === 'openrouter' && !/^sk-or-/i.test(key)) {
    return {
      ok: false,
      error:
        'OpenRouter keys start with sk-or-. You may have saved a different provider’s key under OpenRouter.',
    };
  }
...
function mapProviderError(provider, err) {
  ...
  if (
    ...
    return (
      'OpenRouter rejected this key (User not found). Re-save an OpenRouter key (sk-or-…) under the OpenRouter provider — not an OpenAI/Anthropic/Google key — then Test again.'
    );
  }
  if (lower.includes('incorrect api key') || lower.includes('invalid api key')) {
    return 'Provider rejected the API key. Re-paste the key for the selected provider and save again.';
  }
  ...
  return msg || 'Provider connection failed';
}
...
async function testByokProvider(uid, provider, overrideKey) {
  ...
  try {
    let result;
    if (p === 'openrouter') {
      // Auth probe gives a clearer failure than a chat completion.
