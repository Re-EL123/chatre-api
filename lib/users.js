...
if (!memory.mcp) memory.mcp = new Map();
...
function mcpRef(uid) {
  return userRef(uid).collection('secrets').doc('mcp');
}
...
function byokConfiguredFlags(doc) {
  const { BYOK_PROVIDER_IDS } = require('./providers/byok-providers');
  const out = {};
  BYOK_PROVIDER_IDS.forEach((p) => {
...
async function getMcpDoc(uid) {
  if (getBackend() === 'firestore') {
    const doc = await mcpRef(uid).get();
    if (!doc.exists) return {};
    return doc.data() || {};
  }
  return memory.mcp.get(uid) || {};
}

async function setMcpConnection(uid, uuid, entry) {
  const cur = await getMcpDoc(uid);
  const next = { ...cur, [uuid]: entry, updatedAt: nowIso() };
  if (getBackend() === 'firestore') {
    await mcpRef(uid).set(next, { merge: true });
  } else {
    memory.mcp.set(uid, next);
  }
  return next;
}

async function deleteMcpConnection(uid, uuid) {
  const cur = await getMcpDoc(uid);
  if (cur[uuid]) delete cur[uuid];
  cur.updatedAt = nowIso();
  if (getBackend() === 'firestore') {
    await mcpRef(uid).set(cur);
  } else {
    memory.mcp.set(uid, cur);
  }
  return cur;
}
