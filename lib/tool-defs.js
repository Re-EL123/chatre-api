 ...
 * Long descriptions live in lib/tool-prompts/*.txt (applied below).
 */

const { applyToolPrompts } = require('./tool-prompts/load');
...
const TOOL_DEFS_RAW = [
  ...
  },
  {
    ...
    function: {
      name: 'read_file',
      description:
        'Read a file from the workspace. Prefer start_line/end_line (1-indexed) for large files instead of rewriting whole modules.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string' },
          ...
          start_line: {
            type: 'integer',
            description: '1-indexed inclusive start line (optional)',
          },
          end_line: {
            type: 'integer',
            description: '1-indexed inclusive end line (optional)',
          },
          offset: {
            type: 'integer',
            description: '0-indexed line offset alternative to start_line',
          },
          limit: {
            type: 'integer',
            description: 'Max lines to return when using offset/start_line',
          },
        },
      },
    },
  },
  {
    ...
    function: {
      ...
      description:
        'Return structural outline (functions/classes/types with line numbers) without full file body. Use before ranged read_file or patch_file.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string' },
          file: { type: 'string' },
          max: { type: 'integer', description: 'Max outline entries (default 80)' },
        },
      },
    },
  },
  {
    ...
    function: {
      name: 'resolve_symbol',
      description:
        'Resolve a symbol name to file paths and line ranges via the workspace AST/import graph.',
      parameters: {
        type: 'object',
        properties: {
          symbol: { type: 'string' },
  ...
  },
  {
    type: 'function',
    function: {
      name: 'view_import_graph',
      description:
        'Show import/importer neighborhood for a file from the workspace import graph.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string' },
          file: { type: 'string' },
          depth: { type: 'integer', description: '1–5 (default 1)' },
          direction: {
            type: 'string',
            description: 'imports | importers | both',
          },
        },
      },
    },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Mark an existing workspace file for download (returns path + artifact).',
      parameters: {
        type: 'object',
  ...
  },
  {
    ...
    function: {
      name: 'search_code',
      description:
        'Search file contents with ripgrep on the repo cache (falls back to in-memory scan).',
      parameters: {
        type: 'object',
        properties: {
          pattern: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      name: 'git_clone',
      description:
        'Clone a git remote into /home/user/projects/<slug>/ (depth 1). Prefer for GitHub URLs.',
      parameters: {
        type: 'object',
        properties: {
          url: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      name: 'repo_open',
      description: 'Alias for git_clone — open/refresh a remote repo in the workspace cache.',
      parameters: {
        type: 'object',
        properties: {
          url: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      name: 'run_tests',
      description:
        'Detect and run project tests (npm test / lint / pytest). Sets testsOk for done gates.',
      parameters: {
        type: 'object',
        properties: {
          cmd: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Optional tsc --noEmit / eslint if configs are present.',
      parameters: { type: 'object', properties: {} },
    },
  },
  {
    ...
    function: {
      name: 'git_diff',
      description: 'Show git diff (working tree or --cached) when a real repo is open.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'End-to-end debug a project, open a live localhost preview for the user, and return errors. Required before claiming a build is done. Prefer path=/home/user/projects/<slug>.',
      parameters: {
        type: 'object',
        properties: {
          path: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      name: 'git_init',
      description: 'Initialize git (real on disk when possible; pass real:true to force).',
      parameters: { type: 'object', properties: {} },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      name: 'git_status',
      description: 'Show real git status when repo mode; else simulated git JSON.',
      parameters: { type: 'object', properties: {} },
    },
  },
  {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Run a shell command (modes: workspace|sandbox|local). Prefer cwd over cd &&. Not for file ops — use read_file/write_file/find_files/search_code.',
      parameters: {
        type: 'object',
        properties: {
          ...
          cwd: {
            type: 'string',
            description:
              'Working directory (prefer this over cd &&). Alias: workdir.',
          },
          workdir: {
            type: 'string',
          ...
          timeoutMs: { type: 'number' },
          timeout: { type: 'number', description: 'Alias for timeoutMs' },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Test a saved BYOK provider key with a tiny chat request (openrouter|aihubmix|zai|groq|deepseek|modelscope|ollama|kilo|cloudflare|llm7|ovhcloud|huggingface|dashscope|mistral|xai|anthropic|openai|google|cursor).',
      parameters: {
        type: 'object',
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Search available MCP connectors (Jira, Slack, Notion, GitHub, …) by product or task.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Present MCP connector options with Connect/Use buttons — pass directory UUIDs from search_mcp_registry. End turn after calling.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'call_mcp',
      description:
        'Call a tool on a connected MCP server (server uuid, tool name, arguments). Use list_mcp_tools first.',
      parameters: {
        type: 'object',
        properties: {
          server: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      name: 'list_mcp_tools',
      description: 'List tools exposed by a connected MCP server.',
      parameters: {
        type: 'object',
...

  {
    ...
    function: {
      ...
      description:
        'Create a GitHub pull request for the active repo using the user linked GitHub connector. Pass title, optional body/head/base.',
      parameters: {
        type: 'object',
        properties: {
          title: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'List GitHub pull requests for the active repo.',
      parameters: {
        type: 'object',
        properties: {
          state: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      name: 'get_ci_status',
      description:
        'Fetch GitHub Checks / commit status for a branch or SHA. Sets ciOk.',
      parameters: {
        type: 'object',
        properties: {
          ref: { type: 'string' },
  ...
  },
  {
    ...
    function: {
      name: 'delegate_task',
      description:
        'Spawn nested agent(s) for focused subgoals. agent=explore|general|verify. For parallel independent explores pass goals:[{goal,agent:"explore"},…]. Returns structured summary/findings/filesChanged/risks/nextActions.',
      parameters: {
        type: 'object',
        properties: {
          goal: { type: 'string' },
          ...
          agent: {
            type: 'string',
            description: 'Subagent: explore|general|verify|build or custom id',
          },
          thoroughness: {
            type: 'string',
            description: 'For explore: quick|medium|very thorough',
          },
          goals: {
            type: 'array',
            description:
              'Parallel fan-out (max 4). Each item: {goal, agent?, context?, thoroughness?}. Prefer explore-only for parallel.',
            items: {
              type: 'object',
              properties: {
                goal: { type: 'string' },
...
function flattenParams(item) {
  ...
  if (item.params && typeof item.params === 'object') {
    return { params: { ...item.params }, parseFailed: false };
  }
  // OpenAI / some BYOK models use "parameters" instead of "params"
  if (item.parameters && typeof item.parameters === 'object') {
    return { params: { ...item.parameters }, parseFailed: false };
  }
  if (item.arguments != null) {
    ...
    if (typeof item.arguments === 'object') {
      return { params: { ...item.arguments }, parseFailed: false };
    }
  }
...
let _knownToolNames = null;
function knownToolNames() {
  if (!_knownToolNames) {
    _knownToolNames = new Set(
      TOOL_DEFS_RAW.map((t) => t && t.function && t.function.name).filter(
        Boolean,
      ),
    );
  }
...
/**
 * Map legacy / invented browser tool names onto the real TOOL_DEFS surface.
 * Prompts and models often say screenshot / browser_*; only navigate/computer/read_page exist.
 */
const BROWSER_TOOL_ALIASES = {
  screenshot: (p) => ({
    tool: 'computer',
    params: { ...p, action: p.action || 'screenshot' },
  }),
  browser_navigate: (p) => ({ tool: 'navigate', params: p }),
  browser_screenshot: (p) => ({
    tool: 'computer',
    params: { ...p, action: 'screenshot' },
  }),
  browser_click: (p) => ({
    tool: 'computer',
    params: { ...p, action: p.action || 'left_click' },
  }),
  browser_type: (p) => ({
    tool: 'computer',
    params: { ...p, action: 'type', text: p.text != null ? p.text : p.value },
  }),
  browser_press: (p) => ({
    tool: 'computer',
    params: {
      ...p,
      action: 'key',
      text: p.key != null ? p.key : p.text,
    },
  }),
  browser_scroll: (p) => ({
    tool: 'computer',
    params: { ...p, action: 'scroll' },
  }),
  browser_wait: (p) => ({
    tool: 'computer',
    params: { ...p, action: 'wait', text: p.ms != null ? String(p.ms) : p.text },
  }),
  browser_evaluate: (p) => ({
    tool: 'computer',
    params: {
      ...p,
      action: 'evaluate',
      text: p.script != null ? p.script : p.code != null ? p.code : p.text,
    },
  }),
  browser_read: (p) => ({ tool: 'get_page_text', params: p }),
  browser_content: (p) => ({ tool: 'get_page_text', params: p }),
  browser: (p) => {
    if (p && p.url && !p.action) return { tool: 'navigate', params: p };
    return {
      tool: 'computer',
      params: { ...p, action: (p && p.action) || 'screenshot' },
    };
  },
};

function applyToolAliases(call) {
  if (!call || !call.tool) return call;
  let name = String(call.tool || '').trim();
  // OpenAI / Gemini sometimes emit "functions/read_file" or "tools/read_file"
  const stripped = name.replace(/^(functions|tools)\//i, '');
  if (stripped !== name) {
    name = stripped;
  }
  const map = BROWSER_TOOL_ALIASES[name.toLowerCase()];
  if (!map) {
    if (stripped !== String(call.tool || '').trim()) {
      return { ...call, tool: name, aliasedFrom: call.tool };
    }
    return call;
  }
  const mapped = map(call.params || {});
  return {
    ...call,
    tool: mapped.tool,
    params: mapped.params || {},
    aliasedFrom: call.tool,
  };
}

function normalizeToolName(name) {
  let n = String(name || '').trim();
  if (/^(functions|tools)\//i.test(n)) {
    n = n.replace(/^(functions|tools)\//i, '');
  }
  return n;
}
...
function normalizeToolCall(item) {
  ...
  if (item.function && item.function.name) {
    ...
    return applyToolAliases({
      tool: normalizeToolName(item.function.name),
      params,
      id: item.id || 'tc_' + Math.random().toString(36).slice(2, 8),
      structured: true,
      parseFailed,
      rawArguments: typeof raw === 'string' ? raw : undefined,
    });
  }
  ...
  if (item.name) {
    ...
    if (item.type !== 'function' && !aliasKnown(item.name)) {
      return null;
    }
    const flat = flattenParams(item);
    return applyToolAliases({
...
const TOOL_DEFS = applyToolPrompts(TOOL_DEFS_RAW);
