...
const TEMPLATES = [
  ...
  },
  {
    ...
    done_when: 'Requested files exist and preview_project reports ok',
    files: ['/home/user/projects/app/'],
    success_criteria: [
      'Explore workspace first',
      'Implement requested files',
      'Run preview_project (live localhost) until ok',
    ],
    approach: [
      'view_tree / list_directory',
      'plan + todos',
      'write_file changes',
      'preview_project (fix + live preview)',
    ...
    max_steps: 16,
    executor_brief:
      'Build exactly what was asked. Explore first, implement, call preview_project for live localhost debug, fix errors, then finish with file paths and the preview URL.',
  },
  {
    id: 'document',
...
function enrichBriefing(briefing, userMessage) {
  ...
  if (!isSparseBriefing(b) && String(b.executor_brief || '').trim()) {
    // Keep approved plan fields intact; only backfill missing structure.
    if (!String(b.done_when || '').trim() && Array.isArray(b.success_criteria) && b.success_criteria[0]) {
      b.done_when = String(b.success_criteria[0]);
    }
    if (!Array.isArray(b.files) || !b.files.length) {
      const tpl =
        ...
        null;
      if (tpl && Array.isArray(tpl.files) && tpl.files.length) {
        b.files = tpl.files.slice();
      }
    }
    return b;
  }
