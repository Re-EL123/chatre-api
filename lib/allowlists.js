...
const BY_TYPE = {
  ...
  research: new Set([
    ...
    'csv_query',
    'csv_write',
    // Legacy aliases (normalizeToolCall maps these onto navigate/computer/…)
    'browser_navigate',
    'browser_click',
    'browser_type',
    'browser_press',
...
function assertToolAllowed(tool, taskType, toolsPriority) {
  let name = String(tool || '').trim();
  // Models sometimes emit functions/read_file or tools/read_file
  if (/^(functions|tools)\//i.test(name)) {
    name = name.replace(/^(functions|tools)\//i, '');
  }
