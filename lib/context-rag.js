 ...
 *  - Structural chunking (AST-lite along functions/classes)
 *  - Merkle root over file content hashes (incremental freshness)
 *  - Hybrid search: BM25 + path/symbol boost + optional ripgrep fusion (RRF)
 *  - Priority packing: diagnostics > selection > active > tabs > rg > BM25
 *
 * Gateway: /api/workspace?action=index|context
 * Upgrade path: swap embed()/vector search for Workers AI / Vectorize behind same API.
 */
...
/**
 * Find structural split points (function/class/def starts) for AST-lite chunking.
 */
function structuralBreaks(lines) {
  const breaks = [0];
  const re =
...
function makeChunk(path, lines, start, end) {
...
function chunkFile(path, content, opts) {
  ...
  const breaks = structuralBreaks(lines);
  if (breaks.length >= 2 && lines.length > maxSize) {
    for (let bi = 0; bi < breaks.length; bi++) {
      const start = breaks[bi];
      let end = bi + 1 < breaks.length ? breaks[bi + 1] : lines.length;
      // Cap oversized structural regions with windows
      while (start < end) {
        const sliceEnd = Math.min(end, start + maxSize);
        const c = makeChunk(path, lines, start, sliceEnd);
        if (c) chunks.push(c);
        if (sliceEnd >= end) break;
        start = Math.max(start + 1, sliceEnd - overlap);
      }
      // fix const start mutation — use different loop
    }
  }

  // If structural path failed to produce chunks, or file is small, use windows
  if (!chunks.length) {
    for (let start = 0; start < lines.length; start += Math.max(1, maxSize - overlap)) {
      const end = Math.min(lines.length, start + maxSize);
      const c = makeChunk(path, lines, start, end);
      if (c) chunks.push(c);
...
/** Fixed structural chunker without the buggy loop */
...
function buildIndex(files, opts) {
  ...
  for (const path of paths) {
    ...
    chunks.push(...chunkFileFixed(path, content, opts));
...
/**
 * Reciprocal Rank Fusion of BM25 hits + ripgrep-style lexical hits.
 */
...
function packContext(opts) {
  ...
  hits.forEach((h, i) => {
    ...
    const pri = (h.source === 'rg' ? PRIORITY.rg : PRIORITY.bm25) - i * 0.1;
    addCandidate(
      pri,
      'Retrieved ' +
        h.path +
