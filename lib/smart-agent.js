...
function workspaceInventory(files, opts) {
  ...
  lines.push(
    'Rules: keep one active project; read AGENTS.md before edits; view_tree before editing existing projects; never invent paths (no style.css/script.js unless those files are listed above). For HTML preview use preview_project — not localhost browser tabs.',
  );
  return {
    text: lines.join('\n'),
...
function artifactLooksDone(doneWhen, ctx) {
  ...
  if (/\/home\/user\/projects|index\.html|\.html|write_file|project files/.test(want)) {
    const projectFiles = paths.filter(
      (p) =>
        /\/home\/user\/projects\//.test(p) &&
        !/\/AGENTS\.md$/i.test(p) &&
        String((files[p] && files[p].content) || '').trim().length > 0,
    );
    if (!projectFiles.length) return false;
    const hasHtml = projectFiles.some((p) => /\.html?$/i.test(p));
    const hasCss = projectFiles.some((p) => /\.css$/i.test(p));
    const hasJs = projectFiles.some((p) => /\.(js|mjs|cjs|ts)$/i.test(p));
    const wantsMulti =
      /\b(css|javascript|\.js\b|game|app|calculator|todo|dashboard|website)\b/i.test(
        want,
      );
    if (wantsMulti && hasHtml && !hasCss && !hasJs) {
      const htmlPath = projectFiles.find((p) => /\.html?$/i.test(p));
      const html = String((files[htmlPath] && files[htmlPath].content) || '');
      const selfContained =
        html.trim().length >= 350 &&
