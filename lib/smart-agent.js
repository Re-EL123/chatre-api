...
function workspaceInventory(files, opts) {
  ...
  lines.push(
    'Rules: view_tree or list_directory before first write on build tasks; never invent paths; deliver real files into this workspace.',
  );
  return {
    text: lines.join('\n'),
...
function isDeliverySuccess(tool, result) {
  ...
  const t = String(tool || '');
  return /^(create_pdf|create_document|write_file|append_file|patch_file|upload_artifact|git_commit|execute_command|verify_project)$/.test(
    t,
  );
}
...
function workspacePlaybookExtra(taskType) {
  ...
  if (t === 'document') {
    return (
      common +
      ' Prefer create_pdf/create_document over write_file for user-facing docs. One successful tool = done.'
    );
  }
  if (t === 'build' || t === 'debug') {
    return (
      common +
      ' Scaffold under /home/user/projects/<slug>. After edits run verify_project or execute_command. Do not browse unless required.'
    );
  }
  if (t === 'research') {
    return (
