...
function isDeliverySuccess(tool, result) {
  ...
  const t = String(tool || '');
  return /^(create_pdf|create_document|write_file|append_file|patch_file|apply_patch|upload_artifact|image_generate|text_to_speech|git_commit|execute_command|verify_project)$/.test(
    t,
  );
}
