...
function deriveDoneWhen(briefing, userMessage, taskType) {
  ...
  if (raw && !Understanding.isSoftDoneWhen(raw)) {
  ...
  if (criteria.length && !Understanding.isSoftDoneWhen(criteria[0])) {
  ...
  if (t === 'build' || t === 'debug') {
    if (/\b(html|css|javascript|\.js\b|game|website|webpage)\b/.test(msg)) {
      return 'Project files (e.g. index.html and related assets) exist under /home/user/projects/ from successful write_file tool results and preview_project returns ok';
    }
