...
const PLAYBOOKS = {
  ...
  research:
    'search_web → fetch/read key pages → synthesize with citations. Write a file only if asked.',
  browser:
    'tabs_create → navigate → read_page/find → act with refs. Re-read after navigation.',
  document:
    'If PDF: create_pdf(title, content) with the full body. Else create_document. Do not invent download URLs.',
  build:
    'view_tree/read_file → todos → write_file/patch_file → verify_project or execute_command.',
  debug:
    'Reproduce → read failing files/logs → patch_file → re-run the failing command.',
  ...
  mixed: 'Follow the analyst brief. Prefer the highest-value tool early; verify before finishing.',
};
