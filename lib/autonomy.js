...
const PLAYBOOKS = {
  ...
  question:
    'Answer from knowledge; use search_web/fetch_url only if facts may be stale. Cite sources briefly.',
  research:
  ...
  document:
    'view_tree("/home/user") once if needed, then create_pdf or create_document into /home/user/documents. Apply design (hierarchy, scannable sections). Cite [web:N] when facts came from tools. One successful file tool = done. No browse/code theatre.',
  build:
    'view_tree/list_directory on /home/user/projects → todos → write_file EACH file under /home/user/projects/<slug> (full content) → list_directory confirm → short answer with real paths. Never Python open()/zipfile or chat-only dumps. Apply design (+ web_designs if brand-matched).',
  debug:
    'Systematic debugging: tight red repro loop → root cause → one fix → re-run. Stay in the project tree.',
  git: 'git_status → stage → commit with a clear message. Push only if asked.',
...
function shouldAwaitPlan(autonomy, taskType, isLight) {
  ...
  if (level === 'assist') {
    return ['build', 'debug', 'git', 'run', 'mixed'].indexOf(t) >= 0;
  }
  // ask
  return t !== 'question';
}
...
function preferByokModel(model, byokFlags, defaults) {
  ...
  if (def.model && /^(openrouter|anthropic|openai|google):/i.test(String(def.model))) {
