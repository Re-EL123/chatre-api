...
const PLAYBOOKS = {
  ...
  build:
    'Reflect: confirm deliverable → write PLAN only if still missing → view_tree → write_file under /home/user/projects/<slug> (honor exact user paths; only the files asked for; full content) → preview_project until ok → fix gaps → short answer with real paths. Never Python open()/zipfile or chat-only dumps. Apply design for polished apps (+ web_designs if brand-matched); skip extras for minimal single-file asks.',
  debug:
...
function buildRunMemoryPack({
  ...
  const lines = [
    ...
    'Done when: ' + String(doneWhen || '').slice(0, 400),
    'Team phase: ' + String(phase || '(unset)'),
...
function preferByokModel(model, byokFlags, defaults) {
  ...
  if (def.model && /^(openrouter|aihubmix|zai|groq|deepseek|modelscope|mistral|xai|anthropic|openai|google|cursor|ollama|kilo|cloudflare|llm7|ovhcloud|huggingface|dashscope):/i.test(String(def.model))) {
