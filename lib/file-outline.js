...
/**
 * File outline — AST structural symbols with line numbers for view_file_outline.
 * Prefers ast-parser (brace-aware / optional tree-sitter); falls back to regex.
 */
...
function regexOutline(path, content, max) {
...
function outlineFile(path, content, opts) {
  ...
  try {
    const parsed = AstParser.parseFileSync(path, content);
    if (parsed && Array.isArray(parsed.outline) && parsed.outline.length) {
      items = parsed.outline.slice(0, max).map((x) => ({
        kind: x.kind,
        name: x.name,
        line: x.line,
        endLine: x.endLine || x.line,
        preview: x.preview || '',
      }));
      engine = parsed.engine || 'chatre-ast';
    }
  } catch {
    items = [];
  }
  ...
  if (!items.length) {
    items = regexOutline(path, content, max);
    engine = 'regex';
  }
