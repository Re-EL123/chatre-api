...
function toolAllowlist(agentName) {
  ...
  if (n === 'explore') {
    return /^(read_file|view_file_outline|list_directory|view_tree|find_files|search_code|search_web|fetch_url|http_request|web_extract)$/;
  }
  if (n === 'verify') {
    return /^(read_file|view_file_outline|list_directory|view_tree|find_files|search_code|preview_project|verify_project|execute_command|run_javascript|run_python|execute_code)$/;
  }
  if (n === 'general' || n === 'build') {
    return /^(write_file|append_file|read_file|view_file_outline|list_directory|view_tree|find_files|search_code|create_directory|patch_file|apply_patch|execute_code|run_javascript|run_python|execute_command|create_document|create_pdf|preview_project|verify_project|todo|todo_write)$/;
  }
  // custom: generous but no nested delegate
  return /^(write_file|append_file|read_file|view_file_outline|list_directory|view_tree|find_files|search_code|create_directory|patch_file|apply_patch|execute_code|run_javascript|run_python|execute_command|create_document|create_pdf|preview_project|verify_project|search_web|fetch_url|http_request)$/;
}
