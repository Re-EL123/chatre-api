...
function runPreviewProject(ctx, params) {
  ...
  // Resolve/coerce so home/user/... and doubled project paths still preview
  try {
    const { resolveWorkspacePath } = require('./paths');
    const resolved = resolveWorkspacePath(root, (ctx && ctx.cwd) || '/home/user');
    if (resolved && resolved.ok && resolved.path) {
      root = resolved.path;
    } else {
  ...
  // If preview path pointed at a file, use its parent directory as root
  const files = (ctx && ctx.files) || {};
  if (files[root] && files[root].type === 'file') {
    root = root.replace(/\/[^/]+$/, '') || '/home/user';
  }
