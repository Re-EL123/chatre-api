...
const SKILL_GUIDES = {
  coding:
    'Skill coding (OpenCode): explore → plan+todos → write_file complete files → verify_project/execute_command → README.',
  documents:
  ...
  project:
    'Skill project (OpenCode Build): explore → plan+todos → implement atomically → verify_project → document → git_commit.',
  browser:
    'Skill browser: browser_navigate → inspect text/links/inputs → browser_click/type/press with CSS selectors → browser_screenshot/read to verify. Pass url on follow-up actions. Prefer http_request for raw APIs.',
  computer:
    'Skill computer: use execute_command for shell, files tools for the workspace, http_request for network, browser_* for interactive web. Plan with todos; verify outputs.',
};
