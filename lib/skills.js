...
function detectSkills(text) {
  ...
  if (
    ...
    skills.add('consistent_delivery');
  }
...
const SKILL_GUIDES = {
  ...
  consistent_delivery:
    'Skill consistent_delivery: Obey the intent contract end-to-end. Prefer patch_file over rewrite; never gut unrelated working code. Re-verify done_when/acceptance after each write. Ask one clarify if unsure — do not invent a new plan mid-run. Final answer proves done_when with real paths/tool evidence (no soft "goal completed").',
  shell_debug:
...
/** Catalog of all skills (names + short lines). Full bodies via use_skill. */
function skillCatalogBrief(activeNames, limit) {
  const SelfAwareness = require('./self-awareness');
  return SelfAwareness.formatSkillCatalog(SKILL_GUIDES, activeNames, limit);
}
