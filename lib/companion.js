...
async function runViaBridge({ action, params, tool, companionId, timeoutMs }) {
  ...
  if (!online) {
    return {
      ...
      tool: tool || action,
      error:
        'Desktop companion offline. Start it with: npm run companion:start (set CHATRE_API_BASE + CHATRE_API_TOKEN to bridge). UI shows “Desktop off” until it heartbeats.',
