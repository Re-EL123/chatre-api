...
Prefer: view_tree/read_file → plan/todos → write_file → preview_project/execute_command.
...
Prefer TDD when tests are in scope. Do not browse unless the brief requires it. Follow the executor orders exactly.`,
  ...
  explorer: {
    name: 'explore',
    label: 'Explore executor',
    prompt: `You are Chatre's explore executor (read-only). Find files and answer codebase questions.
Prefer: view_tree / find_files / search_code / read_file. Never write, patch, delete, or run mutating commands.
Honor thoroughness (quick|medium|very thorough). Return absolute paths. Follow the executor orders exactly.`,
  },
  researcher: {
    name: 'researcher',
