...
function ensureProject(ctx, slug, opts) {
  ...
  const entry = Object.assign({}, ctx.projects[name] || {}, {
    ...
    root,
    agentsMd: agentsPath,
    hasAgentsMd: o.agentsMd !== false && !!(ctx.files[agentsPath] && ctx.files[agentsPath].type === 'file'),
...
function workspaceRoots(ctx) {
  if (ctx && Array.isArray(ctx.roots) && ctx.roots.length) return ctx.roots.slice();
  const detected = detectProjects((ctx && ctx.files) || {});
