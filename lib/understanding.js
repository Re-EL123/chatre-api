...
function applyUnderstandingGate(briefing, userMessage, opts) {
  ...
  if (kind === 'deliver' || taskType === 'build' || taskType === 'document') {
    ...
    if (isSoftDoneWhen(b.done_when) && !hasAcceptance) {
      if (unknowns.indexOf('what "done" looks like (observable check)') < 0) {
        unknowns.push('what "done" looks like (observable check)');
      }
      confidence = Math.min(confidence, 0.6);
    }
    if ((!b.goal || String(b.goal).length < 8) && !(Array.isArray(b.files) && b.files.length)) {
  ...
  b.unknowns = asStringList(unknowns);
  b.confidence = confidence;

  const confidenceExplicit =
    b._confidenceExplicit === true ||
    (opts && opts.confidenceExplicit === true);
  // Missing confidence must not force clarify on every turn — only explicit low
  // scores or remaining unknowns.
  const mustClarify =
    b.needs_clarification === true ||
    b.unknowns.length > 0 ||
    (confidenceExplicit && confidence < CONFIDENCE_THRESHOLD);
