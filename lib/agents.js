  ...
  return {
    ...
    research: {
      ...
      prompt:
        ...
        '- For live sites use navigate / read_page / browser_screenshot / browser_* tools and cite evidence\n' +
...
function composerModeBias(mode) {
  ...
  if (m === 'browse') {
    return (
      'Composer mode: Browse — prefer navigate/read_page/screenshot/search_web/fetch_url. ' +
      'Do not scaffold workspace apps unless the user asked to save findings as files.'
    );
  }
  if (m === 'desktop') {
    return (
      'Composer mode: Desktop — prefer desktop_* / companion tools. ' +
      'If the companion is offline, pause and tell the user to start it — do not invent desktop results.'
    );
  }
  if (m === 'code') {
    return (
...
function agentSystemExtra(agent, opts) {
  ...
  if (agent.name === 'research') {
    parts.push(
      'Browser tools are allowed (navigate, read_page, browser_screenshot, browser_*). Prefer them in Browse mode.',
    );
  }
