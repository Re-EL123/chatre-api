...
function resolveAgent({
  ...
  // Composer mode wins over a stale default "build" picker for explore/plan.
  if (mode === 'plan') return map.plan;
  if (mode === 'explore') return map.explore;
  if (mode === 'research') {
    return map.research || map.explore || map.build;
  }
  // browse stays on build (needs navigate/screenshot) — bias comes from composerModeBias.
  if (mode === 'browse') {
    if (requested && map[requested] && requested !== 'plan' && requested !== 'explore') {
