...
const PROMPT_COMPACTION = `You are a context summarization agent. You are given a conversation between a user and an agent plus known run state. Produce a structured Work State summary so another coding agent can continue without re-exploring.

Follow the EXACT section headings requested (Objective, Important Details, Work State with Completed/Active/Blocked, Next Move, Relevant Files). Preserve exact file paths and identifiers. Prefer terse bullets.
...
function nativeAgents() {
  ...
  return {
    ...
    research: {
      name: 'research',
      description:
        'Read-only researcher for planners and Browse mode. Workspace + web + browser; no edits.',
      mode: 'subagent',
      native: true,
      hidden: false,
      color: '#fbbf24',
      ...
      prompt:
        'You are the research agent. Gather facts for strategy/plan, or run Browse-mode sessions.\n\n' +
        'Rules:\n' +
        '- Read-only workspace: no write_file/patch_file/delete/git mutations\n' +
        '- Prefer view_tree, find_files, search_code, read_file, search_web, fetch_url\n' +
        '- For live sites use navigate, read_page, computer(action=screenshot|left_click|type), find, form_input and cite evidence\n' +
        '- Do not scaffold apps or invent desktop results\n' +
        '- Return absolute paths and concrete findings\n' +
        '- Finish with structured JSON (summary, findings[], filesChanged:[], risks[], nextActions[], todos[])',
      whenToUse:
        'Use before planning when the team needs workspace or web facts without making changes, or when composer mode is Browse.',
    },
    strategy: {
      name: 'strategy',
      description:
...
function resolveAgent({
  ...
  // Composer mode wins over a stale default "build" picker for explore/plan/browse.
  if (mode === 'plan') return map.plan;
  if (mode === 'explore') return map.explore;
  if (mode === 'browse' || mode === 'research') {
  ...
  if (mode === 'desktop' || mode === 'code' || mode === 'agent') {
    return map.build;
  }
...
function composerModeBias(mode) {
  const m = String(mode || '').toLowerCase();
  if (m === 'browse') {
    return (
      'Composer mode: Browse — prefer navigate, read_page, computer(action=screenshot|left_click|type), ' +
      'find, form_input, search_web, fetch_url. There is no bare "screenshot" tool. ' +
      'Do not scaffold workspace apps unless the user asked to save findings as files.'
    );
  }
  if (m === 'desktop') {
    return (
      'Composer mode: Desktop — prefer desktop_* companion tools (desktop_status first). ' +
      'If companion_offline, pause and tell the user to run `npm run companion:start` in the chatre1 repo ' +
      'with CHATRE_API_BASE + CHATRE_API_TOKEN — do not invent desktop results.'
    );
  }
  if (m === 'code') {
    return (
      'Composer mode: Code — prefer read_file/patch_file/write_file/run_tests/preview_project. ' +
      'Focus on code quality, diffs, and verification.'
    );
  }
  if (m === 'agent') {
    return 'Composer mode: Agent — full tool loop; deliver workspace proof before claiming done.';
  }
  if (m === 'plan') {
    return 'Composer mode: Plan — write plan markdown only; no product code edits.';
  }
  if (m === 'explore') {
    return 'Composer mode: Explore — read-only search; no writes.';
  }
  return '';
}
...
function agentSystemExtra(agent, opts) {
  ...
  const modeBias = composerModeBias(o.composerMode);
  if (modeBias) parts.push(modeBias);
  ...
  if (agent.name === 'research' || agent.name === 'explore') {
    parts.push('Read-only workspace writes. Feed findings to strategy/plan. No project mutations.');
  }
  if (agent.name === 'research') {
    parts.push(
      'Browser tools are allowed: navigate, read_page, computer(action=screenshot|left_click|type|key|scroll), find, form_input, get_page_text. Prefer them in Browse mode.',
    );
  }
