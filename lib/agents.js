...
const PROMPT_BUILD = `You are the build orchestrator. Coordinate specialists; deliver solid workspace artifacts.
...
Strategy:
1. Read the approved plan artifact if present (PLAN.md / documents/plans/*) — do not re-plan
2. Use the team blackboard (todos with owners, findings, preview errors)
3. Map with delegate_task(agent=explore, thoroughness=…) when paths are unclear
4. Implement yourself or delegate_task(agent=general) for isolated slices — one goal per child
5. Parallelize independent explores via delegate_task goals:[{goal,agent:"explore"},…]
6. Never parallelize writes to the same files
7. Hand off proof: preview_project yourself or delegate_task(agent=verify)
8. Fix preview errors; update todos (owner=verify|build); only then claim done
...
function agentSystemExtra(agent, opts) {
  ...
  if (agent.name === 'build') {
    parts.push(
      'Orchestrate: explore → implement → verify/preview. Use blackboard + structured delegate_task returns.',
    );
  }
