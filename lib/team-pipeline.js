...
const { shouldBuildFastPath } = require('./build-delivery');
...
function shouldMintSpecialist(intake, opts) {
  ...
  // Clear HTML/app fast-path builds (calculator, single page, etc.) — deliver
  // files directly; do not mint a named specialist or invent STRATEGY theatre.
  if (
    shouldBuildFastPath(
      { task_type: task || 'build' },
      String((intake && intake.request) || '') +
        ' ' +
        String((intake && intake.goal) || ''),
    )
  ...
  // Generic template todos alone are not enough — only mint for truly large builds.
  if (
    task === 'build' &&
    todos >= 5 &&
    /\b(multi[- ]?page|full[- ]?stack|saas|platform|monorepo|enterprise)\b/i.test(
      text,
    )
  ) {
    return {
