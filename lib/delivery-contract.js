...
function evaluateOneTest(test, ctx) {
  ...
  const htmlBodies = () =>
    Object.keys(files)
      .filter((p) => files[p] && files[p].type === 'file' && /\.html?$/i.test(p))
      .map((p) => String(files[p].content || ''));
  ...
  // Content assertions: <h1> contains "…"
  const h1Match = String((test && test.text) || '').match(
    /<h1>\s*contains\s+["']([^"']+)["']/i,
  );
  if (h1Match) {
    const needle = h1Match[1];
    const bodies = htmlBodies();
    const ok = bodies.some((html) => {
      const m = html.match(/<h1\b[^>]*>([\s\S]*?)<\/h1>/i);
      return m && m[1].replace(/<[^>]+>/g, '').includes(needle);
    });
    return {
      id: test.id,
      text: test.text,
      pass: ok,
      evidence: ok ? 'h1 contains ' + JSON.stringify(needle) : 'h1 missing ' + JSON.stringify(needle),
    };
  }

  // Content assertions: N <li> elements
  const liMatch = text.match(/(\d+)\s*<li>\s*elements?/);
  if (liMatch) {
    const need = parseInt(liMatch[1], 10);
    const bodies = htmlBodies();
    let count = 0;
    bodies.forEach((html) => {
    ...
    return {
      ...
      text: test.text,
      pass: ok,
      evidence: ok ? count + ' <li> elements' : 'found ' + count + ' <li>, need ' + need,
    };
  }

  if (/opened in a browser|without errors|browser without error/.test(text)) {
