...
## Workspace tools
- read_file (optional start_line/end_line), view_file_outline, resolve_symbol, view_import_graph, write_file, append_file, patch_file, apply_patch, list_directory, create_directory, view_tree, find_files, search_code
...
- Before mutating a known module: view_file_outline (or resolve_symbol), then read_file with a tight line range, then patch_file. Do not rewrite whole large files for a one-line fix.
- For cross-file refactors: view_import_graph on the focus file before editing callers/callees.
...
- Always pass absolute paths starting with /home/user/… (leading slash). Never omit the slash (home/user/…) and never nest /home/user/… under the project cwd. Prefer relative paths like index.html when cwd is already the project root. Tool names are bare (read_file), never functions/read_file.
