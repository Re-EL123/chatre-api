  ...
  if (tool === 'delegate_task') {
    return delegateTaskTool(p, ctx, {
      delegateRun: async ({ goal, context, model, userId, maxSteps }) => {
        const childName = String(p.agent || p.subagent || 'general').toLowerCase();
        const childAgent =
        ...
        const childCtx = Object.assign({}, ctx, {
          ...
          approvedTools: new Set(ctx.approvedTools || []),
        });
        const msgs = [
          {
            role: 'user',
            content:
              ...
              ((childAgent && childAgent.name) || 'general') +
              '). Complete ONLY this goal with tools, then stop.\n\n' +
              Agents.agentSystemExtra(childAgent, {
              ...
              '\n\nPrefer write_file under /home/user/projects when allowed. Do not invent download links.',
          },
        ];
        let last = '';
        let toolsUsed = 0;
        const steps = Math.min(
          Number(maxSteps) || (childAgent && childAgent.steps) || 4,
          6,
        );
        const exploreOnly = childAgent && childAgent.name === 'explore';
        for (let s = 0; s < steps; s++) {
          ...
          try {
            const structured = await chatWorkerWithTools({
              ...
              tools: TOOL_DEFS.filter((t) => {
                ...
                if (exploreOnly) {
                  return /^(read_file|list_directory|view_tree|find_files|search_code|search_web|fetch_url|http_request)$/.test(
                    n,
                  );
                }
                return (
          ...
          if (!tools.length) {
            return {
              ...
              agent: childAgent && childAgent.name,
            };
          }
          msgs.push({ role: 'assistant', content: stepText });
          const results = [];
          for (const call of tools.slice(0, 4)) {
            const result = await executeRemoteTool(call, childCtx);
            toolsUsed += 1;
            results.push({
              tool: call.tool,
