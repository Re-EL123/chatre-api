...
async function runAgentLoop({
  ...
  if (autonomy === 'assist' || autonomy === 'autopilot') {
    approvedTools.add('session');
  }
  if (autonomy === 'autopilot') {
    // still not auto-approve always-ask tools
  }
  ...
  for (let i = startStep; i < iterations; i++) {
    ...
    if (!tools.length) {
      const shouldForceTools = ['build', 'debug', 'document', 'git', 'run'].includes(
      ...
      // Block "I created the files" theatre with zero workspace delivery
      if (
        shouldForceTools &&
        !ctx.deliverySuccess &&
        ...
        ctx._forcedDeliveryNudge = true;
        messages.push({ role: 'assistant', content: finalStepText || '' });
        messages.push({
          role: 'user',
          content:
            '[internal] No workspace files were created yet. Your chat dump does NOT save files. ' +
            'Immediately call write_file (or create_pdf/create_document) with FULL file contents under ' +
            '/home/user/projects/<slug>/ or /home/user/documents/. Then list_directory to confirm. ' +
            'Do not invent Download links. Do not use Python open()/zipfile.',
        });
