  ...
  if (isResume && existingRun && Array.isArray(existingRun.messages)) {
    ...
    if (doPlanHandoff && ctx.briefing) {
      ...
      if (!Array.isArray(ctx.briefing.files) || !ctx.briefing.files.length) {
        const slug = String(ctx.activeProject || ctx.briefing.task_type || 'app')
          .toLowerCase()
          .replace(/[^a-z0-9_-]+/g, '-')
          .replace(/^-+|-+$/g, '')
          .slice(0, 32) || 'app';
      ...
      if (!parsed || !parsed.complete) {
        ...
        if (!fields.doneWhen) fields.doneWhen = ctx.briefing.done_when || 'Goal completed with workspace evidence';
        ...
        if (!fields.steps || fields.steps.length < 2) {
          ...
          while (fields.steps.length < 2) {
            fields.steps.push(fields.steps.length ? 'Verify and summarize' : 'Implement the work');
          }
        }
      ...
      // Seed runtime context the fresh path gets (doneWhen, acceptance, budgets, executor brief).
      {
        ...
        const skillLines = skillNames.length
          ? skillNames.map((n) => SKILL_GUIDES[n] || n).join('\n')
          : '';
      ...
      content:
        ...
        (function ideCtx() {
          ...
          try {
            ...
            if (pack && pack.text) {
              ragBlock = '\n# Retrieved workspace context (RAG)\n' + pack.text;
