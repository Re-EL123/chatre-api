...
async function executeRemoteTool(call, ctx) {
  ...
  if (
    ...
    const params = { ...p };
    if (!params.url && ctx.lastBrowserUrl && tool !== 'browser_navigate') {
      params.url = ctx.lastBrowserUrl;
    }
    const result = await runBrowserTool(tool, params, ctx.browserCookies);
    if (result && result.ok !== false) {
      if (result.url) ctx.lastBrowserUrl = result.url;
      else if (params.url) ctx.lastBrowserUrl = params.url;
      if (Array.isArray(result.cookies)) ctx.browserCookies = result.cookies;
    }
    // Don't feed cookie blobs back into the model transcript
    const forModel = { ...result };
    if (forModel.cookies) {
      forModel.cookieCount = Array.isArray(forModel.cookies)
        ? forModel.cookies.length
        : 0;
      delete forModel.cookies;
    }
    return forModel;
  }
...
async function runAgentLoop({
  ...
  if (isResume && existingRun && Array.isArray(existingRun.messages)) {
    ...
    const brief = workflowBrief(
      autoSkills.length ? 'Focus: ' + autoSkills.join(', ') : '',
    );
    if (brief) {
      messages.push({
        role: 'user',
