...
const ImportGraph = require('./import-graph');
...
const COMPANION_START_HINT =
  'In the chatre1 repo: CHATRE_API_BASE=<api> CHATRE_API_TOKEN=<token> npm run companion:start';

/**
 * Composer mode wins over soft analyst task_type for browse/desktop/explore.
 * Browse must use task_type=browser so allowlists include live browser tools.
 * Pure greetings / answer-only chat must NOT be upgraded to build just because
 * the composer is in Agent mode — that burns the step budget with no reply.
 */
function isPureChatTurn(userMessage, briefing) {
  const msg = String(userMessage || '').trim();
  if (!msg) return false;
  let greeting = false;
  try {
    const sig =
      Understanding.extractIntentSignals &&
      Understanding.extractIntentSignals(msg);
    greeting = !!(sig && sig.greeting);
  } catch (e) {
    greeting = /^\s*(hi|hello|hey|thanks|thank you|yo|sup)\b/i.test(msg) && msg.length < 40;
  }
  if (greeting) return true;
  const tt = String((briefing && briefing.task_type) || '').toLowerCase();
  const dk = String((briefing && briefing.deliverable_kind) || '').toLowerCase();
  const hasTodos = !!(briefing && briefing.todos && briefing.todos.length);
  const hasTools = !!(
    briefing &&
    briefing.tools_priority &&
    briefing.tools_priority.length
  );
  if (
    (tt === 'chat' || dk === 'answer') &&
    !hasTodos &&
    !hasTools &&
    !/\b(build|create|make|fix|write|implement|scaffold|deploy)\b/i.test(msg)
  ) {
    return true;
  }
  return false;
...
function applyComposerModeTaskOverrides(briefing, composerMode, userMessage) {
  if (!briefing) return briefing;
  const m = String(composerMode || '').toLowerCase();
  if (m === 'browse') {
    briefing.task_type = 'browser';
    if (
      !briefing.deliverable_kind ||
      briefing.deliverable_kind === 'answer' ||
      briefing.deliverable_kind === 'chat'
    ) {
      briefing.deliverable_kind = 'mixed';
    }
  } else if (m === 'explore') {
    if (
      briefing.task_type === 'chat' ||
      briefing.task_type === 'question' ||
      briefing.task_type === 'mixed' ||
      !briefing.task_type
    ) {
      briefing.task_type = 'research';
    }
  } else if (m === 'desktop' || m === 'code' || m === 'agent') {
    // Keep greetings / answer-only chat on the light path.
    if (isPureChatTurn(userMessage, briefing)) {
      return briefing;
    }
    if (
      briefing.task_type === 'chat' ||
...
function trimMessages(messages) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    const t = estimateTokens(flat) + 4;
    if (total + t > CONTEXT_SOFT_LIMIT && kept.length > 3) break;
    total += t;
    const normalized = Object.assign({}, msg, { content: flat });
    // Strict gateways reject null content even with tool_calls.
    if (normalized.content == null) normalized.content = '';
    kept.unshift(normalized);
  }
...
function checkpointMessages(messages) {
  ...
  return trimMessages(messages).map((m) => {
    const out = {
      role: m.role,
      content: flattenMessageContent(m.content).slice(0, 12000),
    };
...
async function executeRemoteTool(call, ctx) {
  ...
  // Strip functions./tools. prefixes models sometimes emit
  let tool = String((call && call.tool) || '').trim();
  if (/^(functions|tools)\//i.test(tool)) {
    tool = tool.replace(/^(functions|tools)\//i, '');
    if (call) call.tool = tool;
  }
  ...
  if (tool === 'write_file' || tool === 'create_document' || tool === 'create_pdf' || tool === 'append_file') {
    ...
    if (tool === 'create_document') {
      ...
      if (tool === 'append_file') {
        ...
        // Guard against follow-up rewrites that silently drop form fields.
        const prevFile = ctx.files[filePath];
        if (
          prevFile &&
          prevFile.type === 'file' &&
          ...
          const prev = String(prevFile.content || '');
          const next = String(content || '');
          const countInputs = function (html) {
            return (String(html).match(/<input\b/gi) || []).length;
          };
          const countLabels = function (html) {
            return (String(html).match(/\b(phone|company|status|address|notes?)\b/gi) || [])
              .length;
          };
          const prevInputs = countInputs(prev);
          const nextInputs = countInputs(next);
          if (
            prevInputs >= 3 &&
            nextInputs < prevInputs &&
            nextInputs <= prevInputs - 1 &&
            countLabels(prev) > countLabels(next)
          ) {
            return {
              ok: false,
              tool,
              error:
                'write_file would drop form fields from the existing HTML (had ' +
                prevInputs +
                ' inputs, new has ' +
                nextInputs +
                '). Use read_file then patch_file to preserve phone/company/other fields, or pass forceDropFields=true only if the user asked to remove them.',
              code: 'FIELD_DROP_BLOCKED',
            };
          }
        }
      }
  ...
  if (tool === 'resolve_symbol') {
    const symbol = String(p.symbol || p.name || '').trim();
    if (!symbol) return { ok: false, tool, error: 'symbol required' };
    const graph = ImportGraph.getOrBuild(ctx.files || {}, {
      workspaceId: ctx.workspaceId,
      revision: ctx.revision,
    });
    const result = ImportGraph.resolveSymbol(graph, symbol, {
      path: p.path || p.file || '',
      max: p.max,
    });
    const text =
    ...
    return Object.assign({ tool }, result, { text });
  }

  if (tool === 'view_import_graph') {
    const target = p.path || p.file || ctx.activeFile || '';
    if (!target) {
      return { ok: false, tool, error: 'path required (or set an active file)' };
    }
    ...
    const neighborhood = ImportGraph.neighbors(
      graph,
      resolved.path,
      p.direction || 'both',
      p.depth,
    ...
    const text =
      ...
      neighborhood.edges
        .slice(0, 40)
        .map((e) => e.from + ' → ' + e.to + ' (' + (e.kind || 'import') + ')')
        .join('\n');
    return {
      ok: true,
      tool,
  ...
  if (tool === 'search_mcp_registry') {
    const terms = [];
    if (Array.isArray(p.queries)) terms.push(...p.queries);
    if (p.query) terms.push(p.query);
    const connected = ctx.userId
      ? (await McpClient.listConnections(ctx.userId)).map((c) => c.uuid)
      : [];
    ...
    searchTerms.forEach((t) => {
      McpClient.searchRegistry(t, connectedSet).forEach((r) => {
        if (!seen[r.uuid]) {
          seen[r.uuid] = true;
          results.push(r);
        }
      ...
      text:
        results.length
          ? results
              ...
              .join('\n')
          : 'No connectors matched. Answer directly if the task does not need one.',
    };
  }

  if (tool === 'suggest_connectors') {
    const ids = Array.isArray(p.uuids) ? p.uuids : p.uuids ? [p.uuids] : [];
    const connected = ctx.userId
      ? (await McpClient.listConnections(ctx.userId)).map((c) => c.uuid)
      : [];
    const connectedSet = new Set(connected);
    const options = [];
    ids.forEach((id) => {
      const found = McpClient.searchRegistry(String(id), connectedSet)[0];
      if (found && !options.some((o) => o.uuid === found.uuid)) options.push(found);
    });
    if (!options.length) {
      return {
        ok: false,
        tool,
        error:
          'No matching connectors. Call search_mcp_registry first and pass those directory UUIDs.',
      };
    }
    return {
      ok: true,
      tool,
      type: 'connectors',
      question: String(p.question || 'Which of these would you like to connect?'),
      connectors: options,
      text: options.map((o) => o.name + ' (' + o.uuid + ')').join(', '),
    };
  }

  if (tool === 'call_mcp') {
    if (!ctx.userId) {
      return { ok: false, tool, error: 'Signed-in user required for MCP calls' };
    }
    const server = String(p.server || '').trim();
    const mcpTool = String(p.tool || p.name || '').trim();
    if (!server || !mcpTool) {
      return { ok: false, tool, error: 'call_mcp requires server and tool' };
    }
    ...
    if (!result.ok) return { ok: false, tool, server, error: result.error };
    return {
      ok: true,
      tool,
      server,
  ...
  if (tool === 'list_mcp_tools') {
    ...
    const server = String(p.server || '').trim();
    if (!server) return { ok: false, tool, error: 'server required' };
    const result = await McpClient.listMcpTools(ctx.userId, server);
      ...
      text:
        tools.length
          ? tools
              .map(
                (t) =>
                  (t.name || t) + (t.description ? ' — ' + t.description : ''),
              )
              .join('\n')
...
async function runAgentLoop({
  ...
  let existingThreadEarly = null;
  try {
    existingThreadEarly = await getThread(threadId);
  } catch (e) {
    existingThreadEarly = null;
  }
  const priorUnderstanding =
    (existingThreadEarly && existingThreadEarly.lastUnderstanding) ||
    (briefingSeed && briefingSeed.__lastUnderstanding) ||
    null;
  const priorBriefing =
    (priorUnderstanding && priorUnderstanding.briefing) ||
    priorUnderstanding ||
    null;
  const metaActiveForExpand =
    (meta && meta.activeProject) ||
    (priorBriefing &&
      Array.isArray(priorBriefing.files) &&
      priorBriefing.files[0] &&
      (String(priorBriefing.files[0]).match(/\/projects\/([^/]+)/) || [])[1]) ||
    null;
  // Expand short follow-ups using prior ask + active project.
  let userMessage = Understanding.expandFollowUpMessage(
    userMessageRaw,
    Array.isArray(history) ? history : [],
    {
      activeProject: metaActiveForExpand,
  ...
  const ctx = {
    ...
    activeProject: (function () {
      ...
      if (metaActiveForExpand && detected[metaActiveForExpand]) {
        return metaActiveForExpand;
      }
  ...
  if (isResume && existingRun && Array.isArray(existingRun.messages)) {
    ...
    if (ctx._forceBuildFastPath && ctx.briefing) {
      ...
      const existing =
        (ctx.files &&
          (ctx.files[root + '/index.html'] ||
            ctx.files[root + '/' + (target.relativePath || 'index.html')])) ||
        null;
      const existingLen =
      ...
      if (existingLen > 400) {
        emit({
          ...
          text:
            'Project already at ' +
            root +
            ' — resuming tool loop (not re-scaffolding)…',
        });
        ctx._forceBuildFastPath = false;
      } else {
      emit({
        type: 'phase',
        phase: 'execute',
      ...
      } // end else: scaffold on approve (no existing project)
    } // end _forceBuildFastPath
  } else {
    messages = scrubSteerNoise(
      (history || [])
    ...
    let existingThread = existingThreadEarly;
    if (!existingThread) {
      try {
              ...
              content: analystUserPrompt(
                ...
                historySnippet(messages.slice(0, -1)),
                {
                  ...
                  priorBriefing:
                    (existingThread &&
                      existingThread.lastUnderstanding &&
                      existingThread.lastUnderstanding.briefing) ||
                    priorBriefing,
                },
              ),
            },
    ...
    briefing = Understanding.seedBriefingFromPrior(
      briefing,
      (existingThread && existingThread.lastUnderstanding) || priorUnderstanding,
      {
        userMessage: userMessageRaw || userMessage,
        activeProject: ctx.activeProject,
      },
    );
    ...
    const agenticComposerModesEarly = {
      agent: 1,
      plan: 1,
      explore: 1,
      browse: 1,
    ...
    const composerIsAgenticEarly =
      !!agenticComposerModesEarly[String(composerMode || '').toLowerCase()];
    const incrementalOnProjectEarly =
      !!ctx.activeProject &&
      Understanding.isIncrementalFollowUp(userMessageRaw || userMessage, {
        activeProject: ctx.activeProject,
        priorBriefing: priorBriefing,
      });
    let isLightEarly =
      ...
      (briefing.task_type === 'question' &&
        (!briefing.todos || !briefing.todos.length) &&
        (!briefing.tools_priority || !briefing.tools_priority.length));
    const pureChatEarly = isPureChatTurn(
      userMessageRaw || userMessage,
      briefing,
    );
    if (pureChatEarly) {
      isLightEarly = true;
    } else if (composerIsAgenticEarly || incrementalOnProjectEarly) {
      isLightEarly = false;
      if (
        briefing.task_type === 'chat' ||
        briefing.task_type === 'question' ||
        ...
        briefing.task_type = /\b(fix|bug|broken|not working)\b/i.test(
          String(userMessageRaw || userMessage),
        )
          ? 'debug'
          : composerMode === 'explore'
            ...
            : composerMode === 'browse'
              ...
              : 'build';
        briefing.deliverable_kind =
          composerMode === 'explore' || composerMode === 'browse'
            ? 'mixed'
            : 'deliver';
      }
    }
    applyComposerModeTaskOverrides(
      briefing,
      composerMode,
      userMessageRaw || userMessage,
    );
    ...
    // Only skip for true minimal single-file asks — not every "build an app".
    const awaitPlan =
      !skipPlanApproval &&
      approvePlan !== true &&
      shouldAwaitPlan(autonomy, briefing.task_type, isLightEarly) &&
      !isMinimalPageRequest(userMessage, briefing.goal);
    ...
    // Seed actionable todos for builds so the harness plans next moves.
    // Never seed build todos for greetings / pure chat.
    if (
      ['build', 'debug', 'mixed'].indexOf(String(briefing.task_type || '')) >= 0 &&
      (!briefing.todos || !briefing.todos.length) &&
      !isMinimalPageRequest(userMessage, briefing.goal) &&
      !isPureChatTurn(userMessageRaw || userMessage, briefing)
    ) {
      briefing.todos = [
        {
          id: 'plan-layout',
          content: 'Decide file layout (HTML + CSS/JS as needed) under /home/user/projects/<slug>/',
          status: 'pending',
        },
        {
          id: 'write-entry',
          content: 'Write complete entry HTML with real content (not a stub)',
          status: 'pending',
        },
        {
          id: 'write-assets',
          content: 'Add styles/scripts (files or substantial inline) required by the goal',
          status: 'pending',
        },
        {
          id: 'verify-preview',
          content: 'Call preview_project and fix until acceptance passes',
          status: 'pending',
        },
      ];
    }
    ...
    // Re-resolve when composer mode implies a different agent (plan/explore/browse),
    // even if the FE sent a stale default agent=build.
    const modeNeedsResolve = /^(plan|explore|browse|research)$/i.test(
      String(ctx.composerMode || ''),
    );
    if (!agentNameOpt || modeNeedsResolve) {
      ctx.agentInfo = Agents.resolveAgent({
        agentName: modeNeedsResolve ? null : agentNameOpt,
        composerMode: ctx.composerMode,
    ...
    // Browse/Desktop runtime preflight — fail loudly instead of burning steps on dead tools.
    const modeNow = String(ctx.composerMode || composerMode || '').toLowerCase();
    if (modeNow === 'browse' && !String(process.env.CHATRE_WORKER_URL || '').trim()) {
      ctx.browserRuntimeMissing = true;
      emit({
        type: 'error',
        error:
          'CHATRE_WORKER_URL is not configured on the API — Browse cannot open pages until the Worker URL is set.',
      });
    }
    if (modeNow === 'desktop') {
      try {
        const st = await companion.status(ctx.companionId);
        if (!st || !st.online) {
          ctx.companionOffline = true;
          emit({
            type: 'error',
            error:
              'Desktop companion offline. ' +
              COMPANION_START_HINT +
              '. UI shows “Desktop off” until it heartbeats.',
        ...
        emit({
          type: 'error',
          error:
            'Desktop companion check failed: ' +
    ...
    // Never light-chat away incremental edits when an active project already exists.
    // Never light-chat when the user explicitly chose an agentic composer mode.
    const agenticComposerModes = {
      agent: 1,
      plan: 1,
      explore: 1,
      browse: 1,
    ...
    const composerIsAgentic = !!agenticComposerModes[String(composerMode || '').toLowerCase()];
    const incrementalOnProject =
      !!ctx.activeProject &&
      Understanding.isIncrementalFollowUp(userMessageRaw || userMessage, {
        activeProject: ctx.activeProject,
        priorBriefing: priorBriefing,
    ...
    let isLight =
      ...
      (briefing.task_type === 'question' &&
        (!briefing.todos || !briefing.todos.length) &&
        (!briefing.tools_priority || !briefing.tools_priority.length));
    const pureChatLate = isPureChatTurn(
      userMessageRaw || userMessage,
      briefing,
    );
    if (pureChatLate) {
      isLight = true;
      briefing.task_type = 'chat';
      if (!briefing.deliverable_kind || briefing.deliverable_kind === 'deliver') {
        briefing.deliverable_kind = 'answer';
      }
    } else if (composerIsAgentic || incrementalOnProject) {
      isLight = false;
      if (
        briefing.task_type === 'chat' ||
        briefing.task_type === 'question' ||
        ...
        briefing.task_type = /\b(fix|bug|broken|not working)\b/i.test(
          ...
          : composerMode === 'explore'
            ...
            : composerMode === 'browse'
              ? 'browser'
              : 'build';
        if (briefing.deliverable_kind === 'answer') {
          briefing.deliverable_kind =
            composerMode === 'explore' || composerMode === 'browse'
              ? 'mixed'
              : 'deliver';
    ...
    messages.push({
      ...
      content:
        ...
        Agents.agentSystemExtra(ctx.agentInfo, {
          thoroughness: ctx.thoroughness,
          taskTypeBias: bias,
          composerMode: ctx.composerMode || composerMode,
        }) +
        (ctx.browserRuntimeMissing
          ? '\n\n# Browser runtime\nOFFLINE: CHATRE_WORKER_URL is not set on the API. Do not call navigate/computer/read_page — tell the user Browse needs the Worker URL configured. Prefer search_web/fetch_url only if that still helps.'
          : '') +
        (ctx.companionOffline
          ? '\n\n# Desktop companion\nOFFLINE: ' +
            COMPANION_START_HINT +
            '. Do not invent desktop results. Tell the user to start the companion, then retry.'
          : '') +
        (skillLines ? '\n\n' + skillLines : '') +
        (designExtra || '') +
        '\n\n# Playbook\n' +
        ...
        (function ideCtx() {
          ...
          try {
            ...
            if (pack && pack.text) {
              ragBlock =
                '\n# Retrieved workspace context (Layer 2 hybrid BM25+dense+rg / priompt)\n' +
                pack.text;
    ...
    if (!messages.some((m) => m && m.role === 'system')) {
      messages.unshift({
        ...
        content:
          CHATRE_AGENT_PROMPT +
          '\n\n' +
          Agents.agentSystemExtra(ctx.agentInfo, { thoroughness: ctx.thoroughness, composerMode: ctx.composerMode }),
      });
    }
    ...
    if (shouldBuildFastPath(briefing, userMessage) && !isLight && !(ctx.agentInfo && (ctx.agentInfo.name === 'plan' || ctx.agentInfo.name === 'explore'))) {
      const targetEarly = resolveProjectTarget(
        userMessage,
        briefing.goal,
        briefing,
        { activeProject: ctx.activeProject },
      ...
      const rootEarly =
        targetEarly.root || '/home/user/projects/' + targetEarly.slug;
      const existingEarly =
        (ctx.files &&
          (ctx.files[rootEarly + '/index.html'] ||
            ctx.files[
              rootEarly + '/' + (targetEarly.relativePath || 'index.html')
            ])) ||
        null;
      const existingEarlyLen =
        existingEarly && existingEarly.type === 'file'
          ? String(existingEarly.content || '').length
          : 0;
      if (existingEarlyLen > 400) {
        emit({
          type: 'phase',
          phase: 'execute',
          text:
            ...
            rootEarly +
            ' — improving with tools (not re-scaffolding)…',
        });
      } else {
      emit({
        type: 'phase',
      ...
      const complexBuild =
        /\b(minecraft|three\.?js|webgl|voxel|multiplayer|physics|clone)\b/i.test(
          String(userMessage || '') + ' ' + String(briefing.goal || ''),
        );
      try {
        ...
        if (isCreditError(err) || isProviderQuotaError(err)) {
          ...
          if (alt && alt !== model) {
            ...
            try {
              ...
              emit({
                ...
                phase: 'execute',
                text: complexBuild
                  ? 'LLM generation unavailable — continuing with the tool loop (no scaffold overwrite)…'
                  : 'LLM generation unavailable — writing a playable scaffold…',
              });
            }
          } else if (provider !== 'chatre') {
            const workersModel = '@cf/meta/llama-3.3-70b-instruct-fp8-fast';
            emit({
              type: 'phase',
              phase: 'execute',
              text:
                'BYOK quota/credits — generating on Workers AI, then writing files…',
            });
            model = workersModel;
            usage.model = model;
            try {
            ...
            try {
              ...
              emit({
                ...
                phase: 'execute',
                text: complexBuild
                  ? 'LLM generation unavailable (credits) — continuing with the tool loop…'
      ...
      if (!parsed || !parsed.files || !parsed.files.length) {
        if (complexBuild) {
          emit({
            ...
            phase: 'execute',
            text:
              'No valid LLM files yet — switching to tool loop to build ' +
              slug +
              ' properly…',
          });
          parsed = { slug: slug, files: [] };
        } else {
      ...
      if (!parsed.files || !parsed.files.length) {
        // Skip write/preview; fall through to tool loop
      } else {
      ...
      if (ctx.deliverySuccess && written.length) {
        ...
        if (previewResult && previewResult.ok) {
          ...
          const completeness = Delivery.assessBuildCompleteness(ctx, briefing);
          const incomplete =
            completeness &&
            ...
            (incomplete
              ? '\n\nStill missing: ' +
                ((completeness.missing || []).slice(0, 4).join(', ') ||
                  completeness.reason) +
                '. Continuing to finish the remaining files.'
              : '\n\nLive preview: `' +
                (previewResult.localhost || 'http://localhost') +
                '` (opened in Preview).\nDebug: passed.') +
            '\n</answer>';
          ...
          messages.push({
            role: 'user',
            content: incomplete
              ? '[internal] Fast-path wrote ' +
                written.length +
                ' file(s) but delivery is incomplete (' +
                (completeness.reason || 'missing files') +
                '). Still need: ' +
                ((completeness.missing || []).slice(0, 4).join(', ') ||
                  'more project files') +
                '. Write the remaining files with FULL content now, then preview_project. Do not claim done.'
      ...
      } // end else: had files to write
      } // end else: no existing project
    }
    ...
    if (isLight) {
      ...
      const failedChat =
        /^I could not complete that:/i.test(cleaned) ||
        /rate limit|quota|credits|retired|billing/i.test(cleaned);
      // Chat/question answers are successful deliveries of text — not workspace proofs.
      // Do NOT fake-success when the model/provider failed.
      ctx.deliverySuccess = !failedChat;
      ctx.previewOk = !failedChat;
      ctx.testsOk = !failedChat;
      ctx.taskType = briefing.task_type || 'chat';
      ctx.doneWhen = failedChat
        ? 'Provider error — reply not delivered'
        : briefing.done_when ||
          (briefing.task_type === 'chat'
      ...
      const diag = buildRunDiagnostics({
        ...
        steps: 1,
        toolsUsed: 0,
        deliverySuccess: !failedChat,
        stopReason: failedChat ? 'provider_error' : 'light_chat',
        primarySkill: primarySkills[0] || null,
        model,
  ...
  for (let i = startStep; i < iterations; i++) {
    ...
    if (!tools.length) {
      ...
      if (gate) {
        ctx._gateContinues = Number(ctx._gateContinues || 0) + 1;
        if (ctx._gateContinues >= 3) {
          const stallAnswer =
            full ||
            cleaned ||
            finalStepText ||
            'Stopped after repeated delivery gates — workspace is incomplete. Continue from Files with the missing pieces.';
          emit({
            type: 'gate',
            step: i + 1,
            hidden: true,
    ...
    if (!ctx.deliverySuccess) {
      ctx.stepsWithoutSuccess += 1;
      ctx.stepsWithoutCompletion += 1;
    } else {
      ctx.stepsWithoutSuccess = 0;
      const completeness = Delivery.assessBuildCompleteness(ctx, ctx.briefing);
      if (
        completeness &&
        !completeness.complete &&
        completeness.reason !== 'non-build'
      ) {
        ctx.stepsWithoutCompletion = Number(ctx.stepsWithoutCompletion || 0) + 1;
        if (
          ctx.stepsWithoutCompletion >= 3 &&
          i < iterations - 1 &&
          !ctx._incompleteNudge
        ) {
          ctx._incompleteNudge = true;
          messages.push({
            role: 'user',
            content:
              '[internal] Delivery incomplete (' +
              (completeness.reason || 'missing files') +
              '). Still need: ' +
              ((completeness.missing || []).slice(0, 4).join(', ') ||
                'more project files') +
              '. Write the remaining files NOW with full content. Do not claim done.',
          });
        } else if (ctx.stepsWithoutCompletion >= 5) {
          const partialMsg =
            'Partial delivery — wrote some files but the project is incomplete' +
            (completeness.missing && completeness.missing.length
              ? ' (missing: ' + completeness.missing.slice(0, 4).join(', ') + ')'
              : '') +
            '. Open Files and continue.';
          return finishRun({
            threadId,
            model: usage.model,
            full: partialMsg,
            ...
            ctx,
            runLog,
            stopReason: 'incomplete_delivery',
            forceFail: false,
          });
        }
      } else {
        ctx.stepsWithoutCompletion = 0;
      }
    }
