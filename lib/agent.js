...
function parseMarkdownToolCalls(text) {
  ...
  // Brace-balanced recovery for bare {"tool":...} (BYOK models often skip fences)
  const raw = String(text || '');
  let i = 0;
  while (i < raw.length) {
    const a = raw.indexOf('{"tool"', i);
    const b = raw.indexOf('{ "tool"', i);
    let at = -1;
    if (a >= 0 && (b < 0 || a < b)) at = a;
    else if (b >= 0) at = b;
    if (at < 0) break;
...
async function executeRemoteTool(call, ctx) {
        ...
        const msgs = [
          {
            role: 'user',
            content:
              'You are a Chatre sub-agent. Complete ONLY this goal with tools, then stop.\n\nGoal: ' +
              goal +
              (context ? '\n\nContext:\n' + context : '') +
              '\n\nWorkspace inventory:\n' +
              workspaceInventory(ctx.files, { limit: 24 }).text +
              '\n\nPrefer write_file under /home/user/projects. Do not invent download links.',
          },
        ];
        let last = '';
        let toolsUsed = 0;
        const steps = Math.min(Number(maxSteps) || 4, 6);
        for (let s = 0; s < steps; s++) {
          ...
          try {
            const structured = await chatWorkerWithTools({
              ...
              maxTokens: 1200,
              agent: true,
              tools: TOOL_DEFS.filter((t) => {
                const n = t.function && t.function.name;
                return (
                  /^(write_file|read_file|list_directory|view_tree|find_files|search_code|create_directory|patch_file|apply_patch|execute_code|run_javascript|run_python|create_document|create_pdf)$/.test(
                    n,
                  )
                );
              }),
          ...
          const tools = parseToolCalls(stepText, structuredCalls);
          last = cleanText(stepText) || last;
          if (!tools.length) {
            return {
              ok: true,
              summary: last || 'Sub-agent finished',
              toolsUsed,
              steps: s + 1,
              text: last || 'Sub-agent finished',
            };
          }
          msgs.push({ role: 'assistant', content: stepText });
          const results = [];
          for (const call of tools.slice(0, 4)) {
          ...
          msgs.push({
            ...
            content:
              ...
              '\n\nContinue or finish with <answer> if done.',
          });
        }
        return {
          ok: true,
          summary: last || 'Sub-agent reached step limit',
          toolsUsed,
          steps,
          text: last || 'Sub-agent reached step limit',
        };
      },
    });
  }
  ...
  if (tool === 'write_file' || tool === 'create_document' || tool === 'create_pdf' || tool === 'append_file') {
    ...
    if (isUnderProjects(filePath) && !/\/AGENTS\.md$/i.test(filePath)) {
      ...
      if (slug) {
        ...
        ctx.cwd = '/home/user/projects/' + slug;
      }
    }
    ...
    if (projectInfo && projectInfo.agentsCreated && ctx.files[projectInfo.agentsMd]) {
      await putFile(workspaceId, ctx.files[projectInfo.agentsMd]);
      if (ctx.files[projectInfo.root]) {
        await putFile(workspaceId, ctx.files[projectInfo.root]);
      }
    }
    await saveSnapshot(workspaceId, {
      files: ctx.files,
  ...
  if (tool === 'git_init') {
    ...
    if (info.agentsCreated && ctx.files[info.agentsMd]) {
      await putFile(workspaceId, ctx.files[info.agentsMd]);
    }
    if (ctx.files[info.root]) await putFile(workspaceId, ctx.files[info.root]);
...
async function runAgentLoop({
  ...
  const ctx = {
    ...
    projects: Object.assign({}, detected, (meta && meta.projects) || {}),
    activeProject:
      (meta && meta.activeProject) ||
      (Object.keys(detected).length === 1 ? Object.keys(detected)[0] : null),
    todos: emptyTodos(),
    browserSessionId: '',
    lastTabId: null,
    lastBrowserUrl: '',
  ...
  if (isResume && existingRun && Array.isArray(existingRun.messages)) {
    ...
    if (awaitPlan) {
      ...
      logEvent(runLog, 'awaiting_plan', { autonomy });
      return {
        response: '',
        steps: 0,
        usage: { runId },
    ...
    if (!messages.some((m) => m && m.role === 'system')) {
      messages.unshift({
        role: 'system',
        content: CHATRE_AGENT_PROMPT,
      });
    }

    // Document fast path: one content generation + create_pdf/create_document (saves tokens)
    if (shouldDocumentFastPath(briefing, userMessage) && !isLight) {
    ...
    if (shouldBuildFastPath(briefing, userMessage) && !isLight) {
      emit({
        type: 'phase',
        phase: 'execute',
      ...
      try {
        genText = await chatWorkerStreaming({
          ...
          model,
          userId,
          maxTokens: Math.max(TOKEN_BUDGETS.document_body || 2200, 3500),
          agent: false,
          onToken: (delta) => {
      ...
      if (ctx.deliverySuccess && written.length) {
        const answer =
          ...
          '\n\nOpen these from the Files panel (downloadable).\n</answer>';
        const diag = buildRunDiagnostics({
          taskType: briefing.task_type,
          doneWhen: ctx.doneWhen,
          steps: 1,
      ...
      // Fall through to normal tool loop if generation failed
      emit({
        type: 'phase',
        phase: 'execute',
  ...
  for (let i = startStep; i < iterations; i++) {
    ...
    if (!tools.length) {
      ...
      if (toolsUsed > 0 && ctx.briefing) {
        ...
        if (!critique.pass && critique.fix_brief && !criticRepaired && i < iterations - 1) {
          criticRepaired = true;
          messages.push({ role: 'assistant', content: finalStepText || '' });
          messages.push({
            role: 'user',
            content:
              'Critic found gaps (one repair pass only):\n' +
              (critique.gaps || []).map((g) => '- ' + g).join('\n') +
              '\n\nFix brief:\n' +
              critique.fix_brief +
              '\n\nWorkspace:\n' +
              workspaceInventory(ctx.files, { limit: 20 }).text +
              '\n\nContinue with tools, then finish WITHOUT further critique loops.',
          });
          await persistCheckpoint(threadId, {
            status: 'running',
            runId,
