...
const { detectSkills, SKILL_GUIDES, pickPrimarySkill } = require('./skills');
const {
  TOKEN_BUDGETS,
  deriveDoneWhen,
  workspaceInventory,
...
async function runAgentLoop({
  ...
  if (isResume && existingRun && Array.isArray(existingRun.messages)) {
    ...
    const autoSkills = pickPrimarySkill(autoSkillsAll, null);
    ...
    // Re-pick skill from final task_type
    const primarySkills = pickPrimarySkill(autoSkillsAll, briefing.task_type);
    ...
    if (!tools.length) {
      ...
      if (i === startStep && !isResume && shouldForceTools && toolsUsed === 0) {
        ...
        messages.push({
          role: 'user',
          content:
            '[internal] Call tools now for this task — do not stop with chat only. Do not narrate this message.',
        });
        await persistCheckpoint(threadId, {
          status: 'running',
          runId,
